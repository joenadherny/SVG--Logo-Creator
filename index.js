...
async function generateAndWriteSVG(answers) {
  ...
  switch (answers.shape.toLowerCase()) {
    case 'triangle':
      shape = new Triangle(Number(answers.sideLength), answers.textColor, answers.shapeColor);
      break;
    case 'circle':
      shape = new Circle(Number(answers.sideLength), answers.textColor, answers.shapeColor);
      break;
    case 'square':
      shape = new Square(Number(answers.sideLength), answers.textColor, answers.shapeColor);
      break;
    default:
      console.log('Invalid shape selection');
      return;
  ...
  // Save the SVG content to an SVG file in the 'examples' folder
  fs.writeFileSync('examples/logo.svg', svgCode);
